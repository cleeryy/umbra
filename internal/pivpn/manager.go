...
func (m *Manager) GetStatus() (*Status, error) {
	...
	if _, err := os.Stat(m.configPath); os.IsNotExist(err) {
		return nil, fmt.Errorf("wireguard config path does not exist: %s", m.configPath)
	}
	...
	// Get client count with proper error handling
	...
	// Sanitize output to prevent command injection in logs
	sanitizedOutput := strings.ReplaceAll(string(output), "\n", " ")
